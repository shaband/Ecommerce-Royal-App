...
@section('page_title' , 'Royal |-_-| List Products')
	...
	<!-- @author Emad Rashad  --> 
		@component('backend.components.breadcrumb')
			...
			@slot('main_short_note')
				...
				Control website Products [ <code>devices</code> , <code>subscriptions</code> ] 
			@endslot
							...
							<table id="e-product-list" class="table table-striped dt-responsive nowrap">
								...
								@if(count($products) > 0 )
									...
									<tr>
										...
										<td >{{ $product->code }}</td>
										<td ><a href="" data-toggle="tooltip" data-placement="top" title="" data-original-title="click to list packages under me -_-" >{{ ucfirst($product->category->title) }}</a></td>
										<td >
											<a href="" data-toggle="tooltip" data-placement="top" title="" data-original-title="click to list products under me -_-" >{{ ucfirst($product->package->title) }}</a>
										</td>
										<td class="pro-name">
											<h6>{{  ucfirst($product->title) }}</h6>
								...
								@else
									...
									<tr>
										<td colspan="8" class="text-center"> No products added till now </td>
									</tr>
